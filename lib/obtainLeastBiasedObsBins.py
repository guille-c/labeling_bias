...
def searchBestObsBins (dataAn, bins_ini, bins_end, dL_ini, dL_end, intrinsic, 
                       observables, y, labels, increasing_bias, log2_bins_int,
                       N_calc, tol):
    if (bins_end - bins_ini) <= 1:
        print "returning"
        return [], [], []
    current_bins = int (np.trunc((bins_end + bins_ini)*0.5))
    
    N_labeled = len(np.where(np.in1d (y, labels))[0])
    N_bins = 2**log2_bins_int*current_bins
    ...
    if obj_per_bin < 10:
        ...
        dLm, dLs = calculateDL (dataAn, current_bins, intrinsic, observables, 
                                y, labels, increasing_bias, log2_bins_int, N_calc)

        print "dL[", current_bins, "] = ", dLm, dLs, bins_ini, bins_end, obj_per_bin
        #print L1

    if dLm >= tol:
        ret_dLm, ret_dLs, ret_bins = searchBestObsBins (dataAn, bins_ini, 
                                                        current_bins, 
                                                        dL_ini, dLm, intrinsic, 
                                                        observables, 
                                                        y, labels, 
                                                        increasing_bias, 
                                                        log2_bins_int, N_calc, tol)
    else:
...
parser.add_argument("--log2_bins_int", default = 3,
                    type = int,
                    help = "log2 bins in intrinsic parameters 'b11 b12;b21 b22.")
# parser.add_argument("--bins_obs_range",
#                     nargs = 2,
...
parser.add_argument("--tol", default = 5E-3, type = float,
...
N_iter = args.N_iter
print args.log2_bins_int
log2_bins_int = int(args.log2_bins_int)
...
bins_min = 2
dL_min, dLs_min = calculateDL (dataAn, bins_min, intrinsic, observables, 
                               y, labels, increasing_bias, log2_bins_int, N_iter)
N_obj_per_bin = 15
bins_max = int (intrinsic.shape[0]/(2.**log2_bins_int)/N_obj_per_bin)
dL_max, dLs_max = calculateDL (dataAn, bins_max, intrinsic, observables, 
                               y, labels, increasing_bias, log2_bins_int, N_iter)

dLm, dLs, bins_obs = searchBestObsBins (dataAn, bins_min, bins_max, dL_min, dL_max, 
                                        intrinsic, observables, y, labels, 
                                        increasing_bias, log2_bins_int, N_iter, tol)

dLm = [dL_min] + dLm + [dL_max]
bins_obs = [bins_min] + bins_obs + [bins_max]
dLm = np.array(dLm)
